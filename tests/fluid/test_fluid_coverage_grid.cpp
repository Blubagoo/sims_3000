/**
 * @file test_fluid_coverage_grid.cpp
 * @brief Unit tests for FluidCoverageGrid (Ticket 6-008)
 *
 * Tests cover:
...
// =============================================================================
// Set and Query Single Cell Tests
// =============================================================================
...
// =============================================================================
// Set Multiple Cells for Same Owner Tests
// =============================================================================

TEST(set_multiple_cells_same_owner) {
    FluidCoverageGrid grid(128, 128);
    grid.set(0, 0, 2);
    grid.set(1, 0, 2);
    grid.set(2, 0, 2);
    ...
    ASSERT_EQ(grid.get_coverage_owner(0, 0), 2);
    ASSERT_EQ(grid.get_coverage_owner(1, 0), 2);
    ASSERT_EQ(grid.get_coverage_owner(2, 0), 2);
    ASSERT_EQ(grid.get_coverage_owner(3, 0), 2);
    ASSERT_EQ(grid.get_coverage_owner(4, 0), 2);
    ASSERT_EQ(grid.get_coverage_count(2), 5u);
}
...
TEST(clear_all_for_owner_removes_only_matching) {
    ...
    // Clear all cells owned by owner 1
...
// =============================================================================
// Multiple Owners on Same Grid Tests
// =============================================================================

TEST(multiple_owners_on_same_grid) {
    FluidCoverageGrid grid(128, 128);
    grid.set(0, 0, 1);
    grid.set(10, 10, 2);
    grid.set(50, 50, 3);
    ...
    ASSERT(!grid.is_in_coverage(0, 0, 2));
    ASSERT(grid.is_in_coverage(10, 10, 2));
    ASSERT(!grid.is_in_coverage(10, 10, 1));
}

TEST(multiple_owners_coverage_count) {
    FluidCoverageGrid grid(128, 128);
    // 5 cells for owner 1
...
// =============================================================================
// Various Map Sizes Tests
// =============================================================================

TEST(map_size_128x128) {
    FluidCoverageGrid grid(128, 128);
    // Set all cells to owner 1 and verify count
    for (uint32_t y = 0; y < 128; ++y) {
        for (uint32_t x = 0; x < 128; ++x) {
            grid.set(x, y, 1);
        }
...
int main() {
    printf("=== FluidCoverageGrid Unit Tests (Ticket 6-008) ===\n\n");
    ...
    // Set multiple cells for same owner
    RUN_TEST(set_multiple_cells_same_owner);
    ...
    // Multiple owners on same grid
    RUN_TEST(multiple_owners_on_same_grid);
    RUN_TEST(multiple_owners_coverage_count);
    ...
    // Various map sizes
    RUN_TEST(map_size_128x128);
    RUN_TEST(map_size_256x256);
    RUN_TEST(map_size_512x512);
